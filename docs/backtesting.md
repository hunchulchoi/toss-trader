...
- 선택한 경우 paper 운영과 같은 종목 수, UTC 일일 매수 횟수, 종목별 주문 체결가
  기준 주문·포지션 notional 한도를 매수 전에 적용한다. 옵션을 생략하면 기존 결과
...
`max_*_rejections`는 각 선택 한도 때문에 건너뛴 신호 수다. 같은 시각에 여러
신호가 있으면 종목코드 순서가 한도 소진 순서도 결정한다.
