...
- 분석 panel은 1d cycle 수익률만 보지 않는다. 같은 서울 일자 `1m`
  `cycle_insight`를 모아 `intradayReview`/`dailyReview`로 넘기고, 벤치마크
  포함 저장된 1분봉으로 `marketContext`를 붙인다. 1d는 결측 시세 분을 채운다.
  `runId=null`은 당일 freeze다. `below-one-lot`은 `armRejectDetail`로 사이징을
  남긴다. 판단 기준은 규칙 준수와 그 시세 대비이지 당일 플러스가 아니다.
