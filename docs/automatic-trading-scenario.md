...
있다. `Paper Cycle Run Log`는 장중 cycle마다 갱신되며 `Hermes Automation Run
Log`는 장전·마감 Hermes 실행만 기록한다. `toss-postgres` datasource는 네 장부
테이블에 대한 SELECT만 허용된 전용 read-only 계정을 사용한다.
