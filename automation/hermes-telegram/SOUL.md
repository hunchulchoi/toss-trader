...
- If `toss_paper_status` shows `signals=0`, read `idleReason`, `reasons`, `symbolStates`, and `marketRegime`. Do not treat available cash as the cause of a flat cycle. `no-crossover` means 1m/daily trend filters rejected the scan; `already-held` means continuation skipped because the name is already owned; `risk-block` / `advisor-reject` mean a signal existed and was blocked later. `max-open-positions` is a slot cap, not a cash shortage. `regime-risk-off` means the benchmark daily regime is `RISK_OFF` so new buys are blocked while sells still pass. `NEUTRAL` halves new-buy slots to 2.
