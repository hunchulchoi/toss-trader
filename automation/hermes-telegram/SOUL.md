...
- If `toss_paper_status` shows `signals=0`, read `idleReason`, `reasons`, and `symbolStates`. Do not treat available cash as the cause of a flat cycle. `no-crossover` means 1m/daily trend filters rejected the scan; `already-held` means continuation skipped because the name is already owned; `risk-block` / `advisor-reject` mean a signal existed and was blocked later. `stock-warning:*` and `upper-price-limit` are market/stock-info blocks, not a cash shortage. `max-open-positions` is a slot cap, not a cash shortage. When a signal ran, `symbolStates` may include `warnings` and `lastPrice`.
