...
class MovingAverageBacktestTest(unittest.TestCase):
    def test_replays_crosses_with_toss_costs(self) -> None:
        result = run_ma_backtest(
            candles=_candles([10000, 10000, 10000, 12000, 12000, 12000, 8000]),
            quantity=Decimal(10),
        ...
        self.assertEqual(result.trades[1].commission, Decimal(12))
        self.assertEqual(result.trades[1].tax, Decimal(160))
        self.assertEqual(result.total_costs, Decimal(190))
        self.assertEqual(result.realized_pnl, Decimal(-40190))
        self.assertEqual(result.unrealized_pnl, Decimal(0))
        self.assertEqual(result.final_equity, Decimal(959810))
        self.assertEqual(result.total_return_rate, Decimal("-0.04019"))
        self.assertEqual(result.max_drawdown_rate, Decimal("0.04019"))
    ...
    def test_marks_open_position_to_last_close(self) -> None:
        ...
        self.assertEqual(result.unrealized_pnl, Decimal(9982))
        self.assertEqual(result.final_equity, Decimal(1009982))
