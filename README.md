...
`/cycles`는 cycle 실행 흐름이다. 시각은 서울 24시, 기본 날짜는 서울 달력 오늘.
`/hermes`는 Hermes 판단·분석 응답이다.
...
종목은 고정 watchlist가 아니다. 08:35 KST에 KRX D-1 시세를 실제 조회해 현재
`market_symbols`와 교차하고, D-1 거래대금·종가와 직전 완결 일봉 200개를
사용한다. STOCK·보통주·ACTIVE·거래정상
종목의 가격 setup과 PIT 수급·이벤트를 먼저 전수 평가하고, 통과자만 D-1 거래대금으로 재랭크해
최대 15개까지 고정한다. `TOP_GAINERS`와 장중 Toss/KRX 순위 변경은 실제 entry
membership에 쓰지 않는다. 부족분을 채우지 않는다. 선정 0종도 정상 성공으로
당일 고정한다. 가격 setup 통과 후보의 필수 PIT 수급·이벤트 결손과 metadata·가격 데이터 오류는
...
가점으로 쓰며 미반전만으로 막지 않는다. D+1 첫 완결 1분봉 시가로 3% 갭과
setup-low 훼손을 확인하고, 각 cycle 최신 완결 1분봉 종가로 위험 수량과
paper 실행가를 다시 계산한다. 최신 완결봉 누락 시 시초가를 재사용하지 않는다.
신규 진입 arm은 개장 후 30분까지만 허용한다. 이후에도 동일 후보가
arm 가능한지는 shadow로 기록하지만 주문·체결은 만들지 않는다. 기존 포지션
청산은 이 시간 제한과 무관하게 계속 평가한다.
시초가가 setup low를 훼손한 후보는 09:15~15:20 모든 재회복과 3봉 유지를
별도 shadow로 기록한다. 09:30 이후 통과는 `invalid-stop-reclaim-late`이며
Rule 진입창·수량·Risk를 바꾸지 않는다.
...
| 평일 10:03~15:03, 매시간 | 저장된 시장·후보·거절 상태 점검 | 새 특이사항만 cutoff 근거로 심층 검토 | 새/변경 특이사항만 전송 |
